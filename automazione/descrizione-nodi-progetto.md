...
heliospoc.terna.it            -> cloud o on-prem, in base allo stato DR
...
- `server-dns`, per aggiornare la zona host-specific `heliospoc.terna.it`;
...
```bash
curl https://heliospoc.terna.it/api/v1/session
```
...
```text
client interno -> DNS aziendale -> heliospoc.terna.it -> sito primario AWS -> helios-web / helios-bff -> RDS PostgreSQL
```

Il flusso principale in DR mode e:

```text
client interno -> DNS aziendale -> heliospoc.terna.it -> k3s-datacenter -> helios-web / helios-bff -> postgres on-prem (database helios)
```
