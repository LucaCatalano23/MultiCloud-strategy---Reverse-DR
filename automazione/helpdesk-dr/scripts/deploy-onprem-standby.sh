...
exec_onprem sh -lc "rm -rf /tmp/helpdesk-dr/kubernetes/onprem/app && mkdir -p /tmp/helpdesk-dr/kubernetes/onprem/app && cp /tmp/helpdesk-dr/app/main.py /tmp/helpdesk-dr/app/requirements.txt /tmp/helpdesk-dr/kubernetes/onprem/app/"

exec_onprem sh -lc "kubectl kustomize --load-restrictor=LoadRestrictionsNone /tmp/helpdesk-dr/kubernetes/onprem | kubectl apply -f -"
